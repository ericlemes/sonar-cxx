...
public class TestComplexFunctionsLoCMeasureComputer {
  ...
  public void testWhenDefiningShouldReturnCorrectDefinition(){
    ...
    verify(this.definitionContextBuilder).setOutputMetrics(FunctionComplexityMetrics.COMPLEX_FUNCTIONS_LOC.key());
    verify(this.definitionContextBuilder).build();
  }
  
  @Test
  public void testWhenCallingComputeShouldComputeSumCorrectly(){        
    MeasureComputerTestHelper.setupComponentAndIntMeasure(context, Component.Type.FILE, 15, FunctionComplexityMetrics.COMPLEX_FUNCTIONS_LOC.key());    
    this.measureComputer.compute(context);
    
    MeasureComputerTestHelper.setupComponentAndIntMeasure(context, Component.Type.FILE, 20, FunctionComplexityMetrics.COMPLEX_FUNCTIONS_LOC.key());    
    this.measureComputer.compute(context);
    
    MeasureComputerTestHelper.setupProject(context);
    this.measureComputer.compute(context);
    
    verify(context).addMeasure(FunctionComplexityMetrics.COMPLEX_FUNCTIONS_LOC.key(), 35);
  }
  ...
  public void testWhenCallingComputeWithFileWithNoMeasureShouldNotThrow(){
    MeasureComputerTestHelper.setupComponentAndIntMeasure(context, Component.Type.FILE, 42, FunctionComplexityMetrics.COMPLEX_FUNCTIONS_LOC.key());    
    when(this.context.getMeasure(FunctionComplexityMetrics.COMPLEX_FUNCTIONS_LOC.key())).thenReturn(null);
    this.measureComputer.compute(context);
