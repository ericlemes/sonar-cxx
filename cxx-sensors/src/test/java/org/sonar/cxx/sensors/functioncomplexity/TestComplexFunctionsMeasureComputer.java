...
public class TestComplexFunctionsMeasureComputer {
  ...
  public void testWhenDefiningShouldReturnCorrectDefinition(){
    ...
    verify(this.definitionContextBuilder).setOutputMetrics(FunctionComplexityMetrics.COMPLEX_FUNCTIONS.key());
    verify(this.definitionContextBuilder).build();
  }
  
  @Test
  public void testWhenCallingComputeShouldComputeSumCorrectly(){        
    MeasureComputerTestHelper.setupComponentAndIntMeasure(context, Component.Type.FILE, 3, FunctionComplexityMetrics.COMPLEX_FUNCTIONS.key());    
    this.measureComputer.compute(context);
    
    MeasureComputerTestHelper.setupComponentAndIntMeasure(context, Component.Type.FILE, 2, FunctionComplexityMetrics.COMPLEX_FUNCTIONS.key());    
    this.measureComputer.compute(context);
    
    MeasureComputerTestHelper.setupProject(context);
    this.measureComputer.compute(context);
    
    verify(context).addMeasure(FunctionComplexityMetrics.COMPLEX_FUNCTIONS.key(), 5);
  }
