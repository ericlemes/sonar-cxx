...
public class TestComplexFunctionsLoCPercMeasureComputer {
 private ComplexFunctionsLoCPercMeasureComputer measureComputer;
  ...
  public void testWhenDefiningShouldReturnCorrectDefinition(){
    ...
    verify(this.definitionContextBuilder).setInputMetrics(FunctionSizeMetrics.LOC_IN_FUNCTIONS.key(), FunctionComplexityMetrics.COMPLEX_FUNCTIONS_LOC.key());
    verify(this.definitionContextBuilder).setOutputMetrics(FunctionComplexityMetrics.COMPLEX_FUNCTIONS_LOC_PERC.key());
    verify(this.definitionContextBuilder).build();
  }
  
  @Test
  public void testWhenCallingComputeShouldComputeCorrectly(){        
    Component file = MeasureComputerTestHelper.setupComponentAndIntMeasure(context, Component.Type.FILE, 100, FunctionSizeMetrics.LOC_IN_FUNCTIONS.key());    
    MeasureComputerTestHelper.addIntMeasure(context, file, 50, FunctionComplexityMetrics.COMPLEX_FUNCTIONS_LOC.key());
    this.measureComputer.compute(context);
    
    Component file2 = MeasureComputerTestHelper.setupComponentAndIntMeasure(context, Component.Type.FILE, 25, FunctionSizeMetrics.LOC_IN_FUNCTIONS.key());    
    MeasureComputerTestHelper.addIntMeasure(context, file2, 5, FunctionComplexityMetrics.COMPLEX_FUNCTIONS_LOC.key());
    this.measureComputer.compute(context);
    
    MeasureComputerTestHelper.setupProject(context);
    this.measureComputer.compute(context);
    
    verify(context).addMeasure(FunctionComplexityMetrics.COMPLEX_FUNCTIONS_LOC_PERC.key(), 44.0d);
   }
 ...
 @Test
  ...
  public void testWhenCallingComputeWithFileWithNoMeasureShouldNotThrow(){
    MeasureComputerTestHelper.setupComponentAndIntMeasure(context, Component.Type.FILE, 42, FunctionComplexityMetrics.COMPLEX_FUNCTIONS.key());    
    when(this.context.getMeasure(FunctionComplexityMetrics.COMPLEX_FUNCTIONS.key())).thenReturn(null);
    this.measureComputer.compute(context);
  ...
  public void testWhenCallingComputeWithFileWithNoLoCInFunctionsShouldReturn0(){
    Component file = MeasureComputerTestHelper.setupComponentAndIntMeasure(context, Component.Type.FILE, 0, FunctionSizeMetrics.LOC_IN_FUNCTIONS.key());    
    MeasureComputerTestHelper.addIntMeasure(context, file, 0, FunctionComplexityMetrics.COMPLEX_FUNCTIONS.key());   
    this.measureComputer.compute(context);
    
    MeasureComputerTestHelper.setupProject(context);
    this.measureComputer.compute(context);
    
    verify(context).addMeasure(FunctionComplexityMetrics.COMPLEX_FUNCTIONS_LOC_PERC.key(), 0d);
  }      
